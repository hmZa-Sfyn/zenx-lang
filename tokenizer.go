...
var tkNames = map[TK]string{
	...
	TK_PRINT:    "print",  TK_PRINTLN:  "println", TK_EXIT:   "exit",  TK_SIZEOF: "sizeof",
	...
	TK_ARROW: "->", TK_DOT: ".", TK_DOTDOT: "..", TK_ELLIPSIS: "...",
...
var keywords = map[string]TK{
	...
	"print": TK_PRINT, "println": TK_PRINTLN, "exit": TK_EXIT, "sizeof": TK_SIZEOF,
	"int":   TK_TYPE_INT,   "float": TK_TYPE_FLOAT, "bool": TK_TYPE_BOOL,
	"str":   TK_TYPE_STR,   "void":  TK_TYPE_VOID,  "char": TK_TYPE_CHAR,
	"ptr":   TK_TYPE_PTR,
}
...
func (t *Tokenizer) nextToken() {
	...
	case '=':
		if t.tryEat('=') { t.push(TK_EQ,  "==", sp) } else { t.push(TK_ASSIGN, "=", sp) }
	case '<':
...
func (t *Tokenizer) lexString(quote rune) {
	...
	for !t.eof() && t.peek(0) != quote {
		...
		if ch == '\\' {
			...
			default:
				warnAt(sp, fmt.Sprintf("unknown escape sequence \\%c", t.peek(0)),
					"valid escapes: \\n \\t \\r \\\\ \\\" \\0 \\a \\b")
				sb.WriteByte('\\')
...
func (t *Tokenizer) lexCharLit() {
	...
	if t.eof() || t.peek(0) != '\'' {
		errAt(sp, "char literal not closed",
			"use single quotes for chars: 'a'  (for strings use double quotes: \"abc\")")
		t.ok = false; return
	}
	t.advance()
	sp.Len = 3
...
func (t *Tokenizer) peek(offset int) rune {
	if t.pos+offset >= len(t.src) { return 0 }
	return t.src[t.pos+offset]
}

func (t *Tokenizer) tryEat(ch rune) bool {
	if !t.eof() && t.peek(0) == ch { t.advance(); return true }
	return false
}
