...
func TypeCheck(prog *Program, src, file string) bool {
	...
	for _, imp := range prog.Imports {
		if imp.IsStd { tc.importMods[imp.Module] = true } else { tc.importPaths[imp.Path] = true }
	}
	...
	for _, s := range prog.Structs {
		...
		for _, f := range s.Fields {
			if seen[f.Name] { errCode("E02", f.Sp, fmt.Sprintf("duplicate field %q in struct %q", f.Name, s.Name), "remove the duplicate"); tc.ok = false }
			seen[f.Name] = true
			if f.Type != nil && f.Type.Kind == TyStruct { tc.validateTypeExists(f.Type, f.Sp) }
		}
	}
	...
	// check bodies
	for _, stmt := range prog.TopStmts { if fn, ok := stmt.(*FnDecl); ok { tc.checkFn(fn) } }
	for _, m := range prog.Methods { tc.checkMethod(m) }
	// check macro bodies
	for _, mc := range prog.Macros { tc.checkMacro(mc) }
	for _, stmt := range prog.TopStmts { if _, ok := stmt.(*FnDecl); !ok { tc.checkStmt(stmt) } }
	return tc.ok
}
...
func (tc *TypeChecker) currentMacro() *MacroDecl {
	if len(tc.macroStack) == 0 { return nil }
...
func (tc *TypeChecker) currentRetType() *ZXType {
	...
	if mc := tc.currentMacro(); mc != nil { return mc.RetType }
	return nil
}
...
func (tc *TypeChecker) currentName() string {
	...
	if mc := tc.currentMacro();  mc != nil { return "macro:" + mc.Name }
	return "<top-level>"
}

func (tc *TypeChecker) checkFn(fn *FnDecl) {
	saved := tc.scope; tc.scope = newScope(saved, "fn")
	tc.fnStack = append(tc.fnStack, fn)
	seen := map[string]bool{}
	for _, p2 := range fn.Params {
		if seen[p2.Name] { errCode("E04", p2.Sp, fmt.Sprintf("duplicate param %q in %q", p2.Name, fn.Name), "rename one"); tc.ok = false }
		seen[p2.Name] = true
...
func (tc *TypeChecker) checkBlockInTry(b *Block) {
	if b == nil { return }
	saved := tc.scope; tc.scope = newScope(saved, "try")
	for _, s := range b.Stmts { tc.checkStmt(s) }
	tc.scope = saved
}
...
func (tc *TypeChecker) checkStmt(n Node) {
	...
	case *MatchStmt:    tc.checkMatch(s)
	case *TryCatchStmt: tc.checkTryCatch(s)
	case *AssignStmt:   tc.checkAssign(s)
	case *DeferStmt:    tc.inferExpr(s.Call)
	case *AssertStmt:
		...
		tc.inferExpr(s.Msg)
	case *SpawnStmt:    tc.inferExpr(s.Call)
	case *ExprStmt:     tc.inferExpr(s.Expr)
	case *PrintStmt:    for _, a := range s.Args { tc.inferExpr(a) }
	case *ExitStmt:     tc.inferExpr(s.Code)
	case *BreakStmt:
		if !tc.scope.inLoop() { errCode("E07", s.Sp, "'break'/'last' outside of a loop", "only use break inside while / for / until loops"); tc.ok = false }
	case *ContinueStmt:
		if !tc.scope.inLoop() { errCode("E08", s.Sp, "'continue'/'next' outside of a loop", "only use continue inside while / for / until loops"); tc.ok = false }
...
func (tc *TypeChecker) checkForRange(s *ForRangeStmt) {
	fromT := tc.inferExpr(s.From); toT := tc.inferExpr(s.To)
...
func (tc *TypeChecker) checkMatch(s *MatchStmt) {
	...
	for _, arm := range s.Arms {
		if arm.Pattern != nil { tc.inferExpr(arm.Pattern) }
		if arm.Guard != nil   { tc.inferExpr(arm.Guard) }
...
func (tc *TypeChecker) checkTryCatch(s *TryCatchStmt) {
	...
	if s.Catch != nil {
		saved := tc.scope; tc.scope = newScope(saved, "block")
		if s.ErrVar != "" { tc.scope.define(s.ErrVar, &VarInfo{Type: TypInt, Sp: s.Sp}) }
		for _, st := range s.Catch.Stmts { tc.checkStmt(st) }
		tc.scope = saved
	}
	if s.Finally != nil { tc.checkBlock(s.Finally) }
}
...
func (tc *TypeChecker) inferExpr(n Node) *ZXType {
	...
	case *TemplateStr: e.Typ = TypStr; for _, part := range e.Parts { if part.IsExpr && part.Expr != nil { tc.inferExpr(part.Expr) } }; return TypStr
	case *CmdExpr:   e.Typ = TypStr; tc.inferExpr(e.Command); return TypStr
	case *ReadFileExpr: e.Typ = TypStr; tc.inferExpr(e.Path); return TypStr
	case *TernaryExpr:
		tc.inferExpr(e.Cond); then := tc.inferExpr(e.Then); tc.inferExpr(e.Else)
		e.Typ = then; return then

	case *Ident:
		vi := tc.scope.lookup(e.Name)
		if vi == nil {
			if bd := LookupBuiltin(e.Name); bd != nil { e.Typ = bd.Ret; return bd.Ret }
	...
	case *IndexExpr:
		obj := tc.inferExpr(e.Obj); idx := tc.inferExpr(e.Idx)
...
func (tc *TypeChecker) inferMacroCall(e *MacroCallExpr) *ZXType {
	...
	if !ok {
		...
		if s := tc.suggestName(e.Name); s != "" { hint = fmt.Sprintf("did you mean %q?", s) }
...
func (tc *TypeChecker) inferMacroChain(e *MacroCallChain) *ZXType {
	...
	for _, step := range e.Steps {
		...
		tc.checkBlock(step.Body)
		lastType = mc.RetType
		if lastType == nil { lastType = TypVoid }
	}
...
func (tc *TypeChecker) inferBin(e *BinExpr) *ZXType {
	lhs := tc.inferExpr(e.LHS); rhs := tc.inferExpr(e.RHS)
	switch e.Op {
	case "==", "!=":
	...
	case "-", "*", "/", "%":
		...
		e.Typ = lhs; if e.Typ == nil || e.Typ.Kind == TyUnknown { e.Typ = TypAny }; return e.Typ
	case "|", "&", "^", "<<", ">>":
...
func (tc *TypeChecker) inferCall(e *CallExpr) *ZXType {
	...
	if fn, ok := tc.fns[fnName]; ok {
		if !fn.Variadic && len(e.Args) != len(fn.Params) {
			...
			minArgs := 0
			for _, p2 := range fn.Params { if p2.Default == nil { minArgs++ } }
	...
	if fnName != "" {
		...
		if vi == nil {
			...
			if suggestion != "" { hint = fmt.Sprintf("did you mean %q? (or declare it as an extern fn)", suggestion) }
...
func (tc *TypeChecker) inferMethodCall(e *MethodCallExpr) *ZXType {
	...
	if structName != "" {
		...
		if len(available) > 0 { hint += fmt.Sprintf(" — available: %s", strings.Join(available, ", ")) }
...
func (tc *TypeChecker) inferPipe(e *PipeExpr) *ZXType {
	...
	for _, step := range e.Steps[1:] { t = tc.inferExpr(step) }
	e.Typ = t; return t
}
...
func (tc *TypeChecker) inferField(e *FieldExpr) *ZXType {
	...
	for _, f := range sd.Fields { if f.Name == e.Field { e.Typ = f.Type; return f.Type } }
...
func (tc *TypeChecker) validateTypeExists(t *ZXType, sp Span) {
	...
	if t.Kind == TyRef || t.Kind == TyArray || t.Kind == TySlice { tc.validateTypeExists(t.Elem, sp) }
}
...
func (tc *TypeChecker) methodsFor(sn string) []string {
	var out []string; prefix := sn + "_"
	for k := range tc.methods { if strings.HasPrefix(k, prefix) { out = append(out, strings.TrimPrefix(k, prefix)) } }
	return out
}
...
func editDistance(a, b string) int {
	...
	for i := 1; i <= la; i++ {
		curr[0] = i
		for j := 1; j <= lb; j++ {
			if a[i-1] == b[j-1] { curr[j] = prev[j-1] } else { curr[j] = 1 + min3(prev[j], curr[j-1], prev[j-1]) }
		}
		prev, curr = curr, prev
	}
	return prev[lb]
