...
type Emitter struct {
	...
	globalBuf  strings.Builder // file-scope helpers — emitted before main
	indent     int
	deferBuf   []string
	tmpCounter int
}
...
func (e *Emitter) emitProgram(prog *Program) {
	...
	if !hasUserMain {
		...
		mainBody := e.sb.String()
		e.sb = savedSB
		e.indent = 0

		if e.globalBuf.Len() > 0 {
			e.ln("/* macro chain block helpers */")
			e.sb.WriteString(e.globalBuf.String())
			e.globalBuf.Reset()
			e.ln("")
		}

		e.ln("int main(int argc, char **argv) {")
		e.sb.WriteString(mainBody)
		e.ln("    return 0;")
		e.ln("}")
	}
}
...
func (e *Emitter) emitMacroChainStmt(chain *MacroCallChain) {
	...
	for _, step := range chain.Steps {
		blks = append(blks, blkInfo{name: e.tmp(), step: step})
	}
	tmp := e.tmp()

	// Emit block helpers to FILE SCOPE via globalBuf
	for _, b := range blks {
		e.gln("static void %s(void) {", b.name)
		savedSB := e.sb
		savedIndent := e.indent
		e.sb = strings.Builder{}
		e.indent = 1
		e.emitBlockStmts(b.step.Body)
		e.globalBuf.WriteString(e.sb.String())
		e.sb = savedSB
		e.indent = savedIndent
		e.gln("}")
		e.gln("")
